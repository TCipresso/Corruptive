...
public class Bishop : MonoBehaviour
{
    ...
    private static Bishop selectedPiece;
    ...
    private bool IsLegalMove(string newPosition)
    {
        ...
        if (Math.Abs(currentRow - newRow) != Math.Abs(currentColumn - newColumn))
        {
            return false;
        }
    ...
    private bool IsPathClear(string start, string end)
    {
        ...
        int columnIncrement = startColumn < endColumn ? 1 : -1;

        char nextColumn = (char)(startColumn + columnIncrement);
        int nextRow = startRow + rowIncrement;

        while (nextColumn != endColumn && nextRow != endRow)
        {
            string position = nextColumn.ToString() + nextRow;
            if (!IsSquareEmpty(position))
            {
                return false;
            }
            nextColumn = (char)(nextColumn + columnIncrement);
            nextRow += rowIncrement;
        }

        
        return IsSquareEmpty(end) || IsOpponentPieceAt(end);
    }
